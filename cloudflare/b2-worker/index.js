...
function applyCorsHeaders(response, request) {
  ...
  if (allowedOrigin) {
    ...
    newHeaders.set(
      ...
      'Content-Length, Content-Range, Date, X-Worker-Cache-Match, X-Worker-Cache-Put, X-Worker-Total-Ms',
    )
  ...
  console.log(
    `[b2-worker] CORS: Origin=${origin || 'None'} -> Allowed=${allowedOrigin || 'None'} for ${request.method} ${new URL(request.url).pathname} (Status: ${response.status})`,
  )
...
export default {
  async fetch(request, env) {
    try {
      ...
      console.error('[b2-worker] Unhandled error:', err)
  ...
  async handleRequest(request, env) {
    ...
    if (
      ...
      (!pathLower.endsWith('.m3u8') && !pathLower.endsWith('.ts'))
    ) {
    ...
    const videoPathMatch = requestUrl.pathname.match(
      /^\/videos\/([a-f0-9]{24})\/.+$/i,
    )
    ...
    if (videoPathMatch) {
      ...
      const { valid, reason } = await verifyPlaybackToken(
        token,
        mediaId,
      ...
      if (!valid) {
        console.log(
          `[b2-worker] Token rejected — ${reason} — path: ${requestUrl.pathname}`,
        )
    ...
    if (url.pathname.endsWith('.ts')) {
      ...
      const totalTime = Date.now() - startTime
      console.log(
        `[b2-worker] Request failed with B2 status ${b2Response.status}`,
      )
      console.log(`Total: ${totalTime}ms`)
